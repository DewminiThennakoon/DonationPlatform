...
include "connection.php";
  if (isset($_POST['submit'])) {
    ...
    //insert data
    $sql = "INSERT INTO events(`theme`, `organization`, `location`, `date`, `sTime`, `eTime`, `mobileNo`) 
    VALUES ('$theme','$organization','$location','$date','$sTime' ,'$eTime','$mobileNo')";
    $result = $conn->query($sql);

    if ($result == TRUE) {
      //display success alert
      echo '<script>
      window.alert("Successfully added event to the list!");
      window.location.href = "eventView.php"
      </script>';
    }else{
      //display error alert
      echo '<script>
      window.alert("Error!Failed to add the event");
       window.location.href = "eventAdd.php"
      </script>';
    }
    $conn->close();
  }
